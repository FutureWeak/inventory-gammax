...
const InventorySlot: React.ForwardRefRenderFunction<HTMLDivElement, SlotProps> = (
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={item.slot <= 5 && isSlotWithItem(item) ? "shortinventory-slot" : "inventory-slot"}
      style={{
        filter:
    ...
    >
              {inventoryType === 'player' && item.slot <= 5 && <div className={isSlotWithItem(item) ? "inventory-slot-number" : "inventory-emptyslot-number"}>{item.slot}</div>}
      {isSlotWithItem(item) && (
        <div
