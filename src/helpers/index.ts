...
export const getItemUrl = (item: string | SlotWithItem) => {
  ...
  if (isObj) {
    ...
    if (metadata?.image) {
        const fullPath = `${imagepath}/${metadata.image}.webp`;
        // controlla se è un URL assoluto (http/https) o un file
        if (metadata.image.startsWith('http://') || metadata.image.startsWith('https://')) {
            return metadata.image; // è già un URL completo
        }
        // se invece è un nome file locale, prova a caricare il .webp
        return fullPath;
    }
